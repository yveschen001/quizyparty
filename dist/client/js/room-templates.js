...
;(function (global) {
  ...
  function getStatusInfo(status, t) {
    ...
    var color = '#6b7280'
    var background = 'rgba(107,114,128,0.16)'
    if (status === 'published') {
      label = t('roomTemplates.status.published')
      color = '#16a34a'
      background = 'rgba(22,163,74,0.16)'
    }
    return { label: label, color: color, background: background }
  }

  function buildMetaHtml(template, lang, t, featureV3) {
    var safeTitle = escapeHtml(
      template && template.title ? template.title : t('questionSets.untitled'),
    )
    ...
    if (featureV3) {
      var meta = ''
      meta += '<div class="stack">'
      meta +=
        '<div class="flex-between"><h3 class="h3" style="margin:0">' +
        safeTitle +
        ...
        statusInfo.label +
        '</span></div>'
      meta += '<div class="meta-row"><span>' + questionText + '</span>'
      if (updatedText) {
        meta +=
          '<span>' + t('profile.rooms.updated').replace('{date}', updatedText) + '</span>'
      }
      meta += '</div></div>'
      return meta
    }
    ...
    metaHtml +=
      '<span style="font-size:0.75rem;padding:2px 10px;border-radius:999px;background:' +
      statusInfo.color +
    ...
    if (updatedText) {
      metaHtml +=
        ...
        t('profile.rooms.updated').replace('{date}', updatedText) +
        '</div>'
    }
    metaHtml += '</div>'
    return metaHtml
  ...
  function buildBuilderActions(template, t, featureV3, rawTitle) {
    ...
    if (featureV3) {
      var modern = '<div data-role="actions" class="actions-row">'
      modern +=
        '<a class="btn-primary" data-action="edit" data-template-id="' +
        escapeHtml(template.id || '') +
        '" data-set-id="' +
        escapeHtml(template.questionSetId || '') +
        '" aria-label="' +
        escapeHtml(editLabel + ' · ' + rawTitle) +
        '">' +
        editLabel +
        '</a>'
      if (template.status === 'published') {
        modern +=
          '<button class="btn" data-action="unpublish" data-template-id="' +
          escapeHtml(template.id || '') +
          '" data-set-id="' +
          escapeHtml(template.questionSetId || '') +
          '" aria-label="' +
          escapeHtml(unpublishLabel + ' · ' + rawTitle) +
          '">' +
          unpublishLabel +
          '</button>'
      } else {
        modern +=
          '<button class="btn" data-action="publish" data-template-id="' +
          escapeHtml(template.id || '') +
          '" data-set-id="' +
          escapeHtml(template.questionSetId || '') +
          '" aria-label="' +
          escapeHtml(publishLabel + ' · ' + rawTitle) +
          '">' +
          publishLabel +
          '</button>'
      }
      modern +=
        '<button class="btn" data-action="delete" data-template-id="' +
        escapeHtml(template.id || '') +
        '" data-set-id="' +
        escapeHtml(template.questionSetId || '') +
        '" aria-label="' +
        escapeHtml(deleteLabel + ' · ' + rawTitle) +
        '">' +
        deleteLabel +
        '</button>'
      modern += '</div>'
    ...
    actionsHtml +=
      '<a class="btn" data-action="edit" data-template-id="' +
      escapeHtml(template.id || '') +
      '" data-set-id="' +
      escapeHtml(template.questionSetId || '') +
  ...
  function buildProfileActions(template, options, t) {
    ...
    html +=
      '<a class="btn" href="' +
      escapeHtml(options && options.editUrl ? options.editUrl : '#') +
      '" style="font-size:0.85rem">' +
      editLabel +
      '</a>'
    if (canShare) {
      html +=
        '<button class="btn" data-action="toggle-share" style="font-size:0.85rem">' +
        shareLabel +
        '</button>'
    }
    html +=
      '<button class="btn" data-action="toggle-delete" style="font-size:0.85rem">' +
      deleteLabel +
      '</button>'
    html += '</div>'
    if (canShare) {
      html +=
        '<div data-role="share-panel" style="display:none;margin-top:4px;padding:12px;border:1px solid #e5e7eb;border-radius:10px;background:#f9fafb">'
      html +=
      ...
      html +=
        '<input data-role="share-input" type="text" value="' +
        escapeHtml(shareUrl) +
        '" readonly style="flex:1;min-width:220px;padding:8px;border:1px solid #d1d5db;border-radius:8px;background:#f3f4f6" />'
      html +=
        '<button class="btn" data-action="copy-link" style="font-size:0.8rem">' +
        copyLabel +
        '</button>'
      html += '</div>'
      ...
      html += '</div>'
      html +=
        '<p data-role="share-feedback" style="margin:8px 0 0;font-size:0.8rem;color:#059669;display:none"></p>'
      html += '</div>'
    } else {
      html += '<p style="margin:0;font-size:0.8rem;opacity:0.7">' + shareDisabledText + '</p>'
    }
    html +=
      '<div data-role="delete-panel" style="display:none;margin-top:4px;padding:12px;border:1px solid #fee2e2;border-radius:10px;background:#fef2f2">'
    ...
    html += '</label>'
    html +=
      '<button class="btn btn-primary" data-action="confirm-delete" disabled style="margin-top:10px;font-size:0.8rem">' +
      deleteAction +
      '</button>'
    html +=
      '<p data-role="delete-feedback" style="margin:8px 0 0;font-size:0.8rem;color:#b91c1c;display:none"></p>'
    html += '</div>'
    return html
  }

  function buildBaseCard(template, featureV3) {
    var card = document.createElement('div')
    if (featureV3) {
      card.className = 'card stack gap-lg'
    } else {
  ...
  RoomTemplates.buildBuilderCard = function (template, options) {
    ...
    var featureV3 = Boolean(options && options.featureV3)
    var card = buildBaseCard(template, featureV3)
  ...
  RoomTemplates.buildPagination = function (info, tFn) {
    ...
    html +=
      '<button class="btn" data-action="prev"' +
      prevDisabled +
      ' style="font-size:0.85rem">' +
      t('roomTemplates.pagination.previous') +
      '</button>'
    ...
    html +=
      ...
      nextDisabled +
      ' style="font-size:0.85rem">' +
      t('roomTemplates.pagination.next') +
      '</button>'
    html += '</div>'
    return html
  }
