...
import { canUseImprove, getUsageSnapshot, recordImproveUsage } from '../../../server/question/ai-usage'
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const questionIdsRaw: unknown = body.questionIds
  ...
  const targetLangRaw: unknown = body.lang
  ...
  const questions = questionIds
    .map((id) => store.get(id))
    .filter((q): q is NonNullable<ReturnType<typeof store.get>> => Boolean(q && q.userId === user.id))
  ...
  try {
    ...
    const now = Date.now()
    const updatedQuestions: Array<{ id: string; question: string; choices: string[]; correctAnswer: string; keywords: string[] }> = []
    ...
    for (const item of improved) {
      ...
      const rawChoices = Array.isArray(item.choices) ? item.choices.map((choice: any) => String(choice)) : []
      ...
      const aiKeywordsRaw = Array.isArray(item.keywords) ? item.keywords.map((kw: any) => String(kw)) : []
