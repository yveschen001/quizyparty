# QuizyParty

派對答題遊戲平台，支持多語言、房間協作、單人遊玩，並整合 RevenueCat 訂閱管理。

## 快速開始

### 環境要求

- Node.js >= 18
- pnpm（推薦）或 npm

### 安裝依賴

```bash
pnpm install
```

### 環境變數配置

1. 複製 `.env.example` 為 `.env.local`：
   ```bash
   cp .env.example .env.local
   ```

2. 編輯 `.env.local`，填入必要的環境變數（至少需要 `OPENAI_API_KEY` 和 `SITE`）

### 啟動開發伺服器

```bash
pnpm dev
```

開發伺服器會在 `http://localhost:4321` 啟動。

### 構建生產版本

```bash
pnpm build
pnpm preview  # 預覽構建結果
```

## 開發指南

### 項目結構

```
QuizyParty/
├── src/
│   ├── components/          # Astro 組件
│   ├── i18n/               # 多語言配置與文案
│   ├── lib/                # 工具函數（billing, locale, seo, intl）
│   ├── pages/              # 頁面路由（[lang]/, api/, robots.txt 等）
│   ├── server/             # 服務端邏輯（room store, entitlement store）
│   ├── styles/             # 全局樣式
│   └── middleware.ts       # 語言檢測與重定向
├── public/               # 靜態資源
│   ├── data/questions/    # 題庫 JSON 檔案
│   └── js/               # 客戶端腳本
├── scripts/              # 工具腳本（i18n, questions, 等）
└── .cursor/rules/        # Cursor 開發規範
```

### 常用命令

#### 開發與構建
- `pnpm dev` - 啟動開發伺服器
- `pnpm build` - 構建生產版本
- `pnpm preview` - 預覽構建結果

#### 代碼品質
- `pnpm typecheck` - TypeScript 類型檢查
- `pnpm lint` - ESLint 檢查
- `pnpm format` - Prettier 格式化
- `pnpm format:check` - 檢查格式化

#### i18n 工具
- `pnpm i18n:all` - 掃描、生成鍵值、檢查一致性（本地）
- `pnpm i18n:all:ci` - CI 版本（使用 pnpm）
- `pnpm i18n:scan` - 掃描代碼中的 i18n 鍵
- `pnpm i18n:keygen` - 生成 TypeScript 鍵值類型
- `pnpm i18n:check` - 檢查所有語言的鍵值一致性
- `pnpm i18n:auto` - 使用 OpenAI 自動翻譯缺失的鍵值
- `pnpm i18n:csv:export` - 導出 i18n 鍵值為 CSV
- `pnpm i18n:csv:import` - 從 CSV 導入 i18n 鍵值

#### 題庫管理
- `pnpm questions:build` - 從 CSV 構建題庫 JSON
- `pnpm questions:gen:pivot` - 生成多語言題庫（以英文為基準）
- `pnpm questions:gen:native` - 生成單語言題庫

#### SEO 檢查
- `pnpm seo:check` - 檢查 SEO 標籤完整性

### 開發規範

所有開發需遵循 `.cursor/rules/` 中的規範：

- **核心守門規範** (`.cursor/rules/000-core-guardrails.mdc`)：TypeScript 嚴格模式、i18n、安全性、效能
- **i18n + SEO 規範** (`.cursor/rules/010-i18n-seo.mdc`)：多語言文案、SEO 最佳實踐
- **安全與隱私** (`.cursor/rules/020-security-and-privacy.mdc`)：數據安全、隱私保護
- **品質流程** (`.cursor/rules/030-quality-process.mdc`)：測試、CI、PR 流程

### 本地開發工具

#### 模擬 VIP 權益（僅開發環境）

```bash
# 使用開發 API 模擬 VIP 狀態
curl -X POST http://localhost:4321/api/dev/entitlements/upsert \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer dev-just-for-local-testing-please-change" \
  -d '{"appUserId":"your-user-id","active":["vip"]}'
```

## 功能規劃與開發路線圖

### 已完成的 MVP 功能 ✅

1. **基礎功能**
   - ✅ 單人遊玩（載入題庫、答題、結算）
   - ✅ 房間系統（建立、加入、狀態同步、主機控制）
   - ✅ 多語言支持（7 種語言：zh-hant, zh-hans, ja, en, ko, de, fr）
   - ✅ SEO 基礎設施（canonical, hreflang, OG, sitemap）
   - ✅ 權益 gating（房間人數限制：免費 4 人，VIP 20 人）

2. **開發環境**
   - ✅ TypeScript 嚴格模式
   - ✅ i18n 工具鏈
   - ✅ 錯誤處理統一
   - ✅ 基礎 UI/UX 優化

### 待實作的功能規劃 📋

#### 第三階段：功能增強（優先）

1. **用戶認證系統** 🔐
   - Google OAuth 登入
   - 社交網路登入（Facebook、Twitter 等）
   - 用戶資料庫（SQLite/PostgreSQL）
   - Session 管理
   - 用戶頭像與資料

2. **首頁功能增強** 🏠
   - 熱門房間展示（按參與人數排序）
   - 最新房間列表（按創建時間排序）
   - 房間搜索功能
   - 房間分類標籤

3. **個人頁面** 👤
   - 用戶資料管理
   - 答分記錄（歷史答題記錄）
   - 自定義題庫管理
   - 統計數據（答對率、參與房間數等）
   - 成就系統

4. **房間功能增強** 🎮
   - 房間列表 API
   - 房間歷史記錄
   - 房間設置（題庫選擇、時間限制、難度設置）
   - 房間分享功能

5. **UI/UX 設計系統** 🎨
   - 完整設計規範（顏色、字體、間距）
   - 組件庫（按鈕、卡片、表單、模態框）
   - 響應式佈局優化
   - 動畫效果（過渡、載入、反饋）
   - 深色模式支持

6. **答分記錄系統** 📊
   - 答題歷史記錄
   - 統計數據分析
   - 排行榜功能
   - 個人最佳記錄

#### 第四階段：品質與效能（優化）

7. **房間持久化**
   - 將 in-memory 改為資料庫存儲
   - 支持房間列表查詢
   - 房間歷史記錄

8. **實時同步升級**
   - 從輪詢改為 WebSocket/SSE
   - 減少延遲和服務器負載

9. **題庫擴充**
   - 多語言題庫完善
   - 分類題庫（一般、VIP 專屬等）
   - 題庫管理工具

10. **測試框架**
    - 單元測試（Vitest）
    - E2E 測試（Playwright）
    - CI 測試流程

11. **效能優化**
    - 代碼分割
    - 圖片優化
    - API 響應快取
    - 輪詢優化

#### 第五階段：部署準備（上線）

12. **部署準備**
    - 環境變數配置
    - 構建優化
    - 錯誤監控
    - 日誌系統

13. **正式部署**
    - Vercel 配置
    - 域名設置
    - SSL、CDN
    - 監控告警

### 常見問題

#### 1. 房間狀態不同步？
- 目前使用輪詢（每 2 秒），後續會升級為 WebSocket/SSE

#### 2. i18n 鍵值缺失？
- 執行 `pnpm i18n:all` 檢查
- 使用 `pnpm i18n:auto` 自動翻譯（需要 OpenAI API Key）

#### 3. 題庫載入失敗？
- 確認 `public/data/questions/{lang}.json` 存在
- 檢查 JSON 格式是否正確

#### 4. 構建失敗？
- 執行 `pnpm typecheck` 檢查類型錯誤
- 執行 `pnpm lint` 檢查代碼規範
- 確認所有 i18n 鍵值完整（`pnpm i18n:check`）

#### 5. 用戶登入功能在哪裡？
- **目前未實作**，規劃在第三階段實作
- 將支援 Google OAuth 和社交網路登入
- 需要先實作用戶資料庫和 Session 管理

#### 6. 首頁熱門房間展示在哪裡？
- **目前未實作**，規劃在第三階段實作
- 需要先實作房間持久化（資料庫存儲）
- 然後才能實現房間列表、排序、搜索功能

#### 7. 個人頁面和答分記錄在哪裡？
- **目前未實作**，規劃在第三階段實作
- 需要先實作用戶認證系統
- 然後建立答分記錄數據模型和存儲

#### 8. 美觀的 UI/UX 在哪個階段落地？
- **目前只有基礎樣式**，完整設計系統規劃在第三階段
- 將包含：設計規範、組件庫、響應式佈局、動畫效果
- 建議在實作用戶認證和個人頁面時同步完善 UI/UX

---

## Secrets 指南（QuizyParty）

### 變數分層

- **瀏覽器可見（`PUBLIC_` 前綴）**：純展示/不花錢/無權限（如站台名、功能旗標）。  
- **伺服器私密（無 `PUBLIC_`）**：OpenAI、Webhook 驗證、Google/Apple 憑證等，**只放後端/CI**。

### 推薦清單

- `OPENAI_API_KEY`（後端/CI，❌不可暴露）
- `RC_WEBHOOK_AUTH`（後端/CI；RevenueCat Webhook 會帶 Authorization header，伺服器需驗證）  
- `RC_ENTITLEMENT_KEY`（例：`vip`，✨可暴露）
- `RC_IOS_PUBLIC_API_KEY` / `RC_ANDROID_PUBLIC_API_KEY`（RC 的「公開」SDK key，✅可打包）
- `GOOGLE_SA_JSON`（Google Play 服務帳戶 JSON，後端/CI，❌）
- `APPLE_ISSUER_ID` / `APPLE_KEY_ID` / `APPLE_PRIVATE_KEY`（App Store Server API，用於簽 JWT，後端/CI，❌）
- `SITE`（如 `https://quizyparty.com`，✅可暴露）

> RevenueCat 官方建議：Webhook 觸發後**再調 RC REST API**同步最新狀態（常見為 `GET /v1/subscribers/{app_user_id}`）。:contentReference[oaicite:1]{index=1}

### `.env` 與部署

- 版本控制：提交 `.env.example`（示例值），**勿提交** `.env.local`。  
- GitHub Actions：在 repo **Settings → Secrets and variables → Actions → New repository secret** 新增上述變數；Workflow 用 `${{ secrets.VAR_NAME }}` 注入。  

（備註：RevenueCat v1 GET /subscribers 仍是後端同步常用作法；v2 能力逐步擴充，但官方建議 webhook 後打 REST 拉齊資料依然適用。
community.revenuecat.com
+1
）


